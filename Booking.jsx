...
export function canAccess(user, toolId, companySlug) {
  ...
  if (resolvedId.startsWith('company:')) {
    ...
    // §3: Playbook is OVMG-internal. Gate EVERY playbook surface — the top-level
    // tab (handled below via NEVER_OVERRIDABLE) and the per-company sub-tab — to
    // @onevibemediagroup.com emails or admins. A @gmail.com / other-domain member
    // who gains company access still cannot open that company's Playbook, and
    // there is no per-tool override that can grant it to a non-admin.
    if (subtab === 'playbook' && !isOvmg && !user.isAdmin) return false;
