...
// A step can have its own drawn region (step.zoom), an explicit "zoom out
// here" marker (step.zoomReset), or neither (in which case it silently
// inherits whatever zoom state was active on the previous step when played
// back).
function renderZoomViewport(step) {
  // Remove existing indicators
  const existing = document.getElementById('__zoom_viewport');
  if (existing) existing.remove();
  const existingReset = document.getElementById('__zoom_reset_badge');
  if (existingReset) existingReset.remove();

  updateZoomResetButton(step);

  if (step.zoomReset) {
    const badge = document.createElement('div');
    badge.id = '__zoom_reset_badge';
    badge.className = 'zoom-reset-badge';
    badge.innerHTML = `
      <span>Zoomed out from here</span>
      <button class="zoom-reset-clear" title="Undo zoom-out">✕</button>
    `;
    badge.querySelector('.zoom-reset-clear').addEventListener('click', (e) => {
      e.stopPropagation();
      pushUndo();
      delete step.zoomReset;
      persist(false);
      renderZoomViewport(step);
    });
    els.canvas.appendChild(badge);
    return;
  }
...
function updateZoomResetButton(step) {
  if (!els.zoomResetBtn) return;
  els.zoomResetBtn.classList.toggle('active', !!step.zoomReset);
  els.zoomResetBtn.textContent = step.zoomReset ? 'Zoomed out (click to undo)' : 'Zoom out here';
}

els.zoomResetBtn && els.zoomResetBtn.addEventListener('click', () => {
  const step = STATE.steps[STATE.currentIndex];
  if (!step || STATE.showingIntro) return;
  pushUndo();
  if (step.zoomReset) {
    delete step.zoomReset;
  } else {
    delete step.zoom; // a region and an explicit reset are mutually exclusive
    step.zoomReset = true;
  }
  persist(false);
  renderZoomViewport(step);
...
function positionTooltip(step) {
  ...
  const boxWidthPx = step.tooltipWidth || 280;
  tt.style.width = boxWidthPx + 'px';
  ...
  // Edge-aware placement: mirrors the same flip/clamp logic used in the
  // exported player (and confirmed against Storylane's own live behavior) so
  // what you see in the editor matches what ships — a tooltip pinned blindly
  // to the stored side can run off the canvas or sit on top of its own
  // target when the hotspot is near an edge.
  const offset = 24;
  const margin = 2;
  const container = els.canvas || els.canvasWrap;
  const cw = (container && container.clientWidth) || 1;
  const ch = (container && container.clientHeight) || 1;
  const boxWPct = (boxWidthPx / cw) * 100;
  const estBoxHPx = 130;
  const boxHPct = (estBoxHPx / ch) * 100;

  let finalPos = tp || 'bottom';
  if (finalPos === 'right' && (step.clickX + (offset / cw * 100) + boxWPct) > (100 - margin)) {
    finalPos = 'left';
  } else if (finalPos === 'left' && (step.clickX - (offset / cw * 100) - boxWPct) < margin) {
    finalPos = 'right';
  } else if (finalPos === 'bottom' && (step.clickY + (offset / ch * 100) + boxHPct) > (100 - margin)) {
    finalPos = 'top';
  } else if (finalPos === 'top' && (step.clickY - (offset / ch * 100) - boxHPct) < margin) {
    finalPos = 'bottom';
  }

  let anchorY = step.clickY;
  if (finalPos === 'left' || finalPos === 'right') {
    const halfHPct = boxHPct / 2;
    if (anchorY - halfHPct < margin) anchorY = halfHPct + margin;
    else if (anchorY + halfHPct > 100 - margin) anchorY = 100 - halfHPct - margin;
  }
  let anchorX = step.clickX;
  if (finalPos === 'top' || finalPos === 'bottom') {
    const halfWPct = boxWPct / 2;
    if (anchorX - halfWPct < margin) anchorX = halfWPct + margin;
    else if (anchorX + halfWPct > 100 - margin) anchorX = 100 - halfWPct - margin;
  }

  switch (finalPos) {
    case 'top':
      tt.style.left = anchorX + '%';
...
document.addEventListener('mouseup', (e) => {
  ...
  if (STATE.drawing) {
    ...
    if (d.tool === 'zoom') {
      ...
      pushUndo();
      step.zoom = { x, y, w, h };
      delete step.zoomReset; // drawing a region overrides any explicit zoom-out marker
      setTool('select');
      persist(false);
      renderZoomViewport(step);
      return;
...
function positionTooltip(s) {
  ...
  const boxWidthPx = s.tooltipWidth || 280;
  tooltip.style.width = boxWidthPx + 'px';
  ...
  // Edge-aware placement: a tooltip pinned blindly to the stored side ('left'/
  // 'right'/'top'/'bottom') can run off the frame or sit on top of the very
  // button it's pointing at when the hotspot itself is near an edge (e.g. a
  // hotspot at clickX=90% with position 'right' has nowhere to grow into).
  // Storylane's own player auto-flips to whichever side actually has room —
  // mirror that here instead of trusting the stored side unconditionally.
  const offset = 28;
  const margin = 2; // % margin kept clear of the frame edge
  const cw = (zoomContent && zoomContent.clientWidth) || (document.getElementById('frame') || {}).clientWidth || 1;
  const ch = (zoomContent && zoomContent.clientHeight) || (document.getElementById('frame') || {}).clientHeight || 1;
  const boxWPct = (boxWidthPx / cw) * 100;
  // Real box height varies with caption length; this is just a safety margin
  // used to keep the box from being clipped against the top/bottom edge, so
  // erring a bit generous is fine.
  const estBoxHPx = 130;
  const boxHPct = (estBoxHPx / ch) * 100;

  let finalPos = tp || 'bottom';
  if (finalPos === 'right' && (s.clickX + (offset / cw * 100) + boxWPct) > (100 - margin)) {
    finalPos = 'left';
  } else if (finalPos === 'left' && (s.clickX - (offset / cw * 100) - boxWPct) < margin) {
    finalPos = 'right';
  } else if (finalPos === 'bottom' && (s.clickY + (offset / ch * 100) + boxHPct) > (100 - margin)) {
    finalPos = 'top';
  } else if (finalPos === 'top' && (s.clickY - (offset / ch * 100) - boxHPct) < margin) {
    finalPos = 'bottom';
  }

  // For left/right placements, nudge the vertical center away from the top/
  // bottom edge so the box itself never gets clipped there either (this is
  // what the corner-case hotspots, e.g. near y=0, need on top of the flip).
  let anchorY = s.clickY;
  if (finalPos === 'left' || finalPos === 'right') {
    const halfHPct = boxHPct / 2;
    if (anchorY - halfHPct < margin) anchorY = halfHPct + margin;
    else if (anchorY + halfHPct > 100 - margin) anchorY = 100 - halfHPct - margin;
  }
  let anchorX = s.clickX;
  if (finalPos === 'top' || finalPos === 'bottom') {
    const halfWPct = boxWPct / 2;
    if (anchorX - halfWPct < margin) anchorX = halfWPct + margin;
    else if (anchorX + halfWPct > 100 - margin) anchorX = 100 - halfWPct - margin;
  ...
  switch (finalPos) {
    case 'top': tooltip.style.left = anchorX + '%'; tooltip.style.top = 'calc(' + s.clickY + '% - ' + offset + 'px)'; tooltip.style.transform = 'translate(-50%, -100%)'; break;
    case 'bottom': tooltip.style.left = anchorX + '%'; tooltip.style.top = 'calc(' + s.clickY + '% + ' + offset + 'px)'; tooltip.style.transform = 'translateX(-50%)'; break;
    case 'left': tooltip.style.left = 'calc(' + s.clickX + '% - ' + offset + 'px)'; tooltip.style.top = anchorY + '%'; tooltip.style.transform = 'translate(-100%, -50%)'; break;
    case 'right': tooltip.style.left = 'calc(' + s.clickX + '% + ' + offset + 'px)'; tooltip.style.top = anchorY + '%'; tooltip.style.transform = 'translateY(-50%)'; break;
  }
}
