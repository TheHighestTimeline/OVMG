...
export default function Dashboard({ user, onLogout }) {
  ...
  const resolveView = () => {
    ...
    const VIEWS = {
      ...
      'calendar':   gateView('calendar',   <Calendar   {...ctx} />),
      'booking':    gateView('booking',    <Booking    {...ctx} />),
      'tools':      gateView('tools',      <Tools      {...ctx} />),
      'playbook':   gateView('playbook',   <Playbook   {...ctx} />),
  ...
  const isFullBleed = isSocial || isTools || isPlaybook;
    ...
    <div style={{ display: 'flex', height: '100vh', fontFamily: SANS, background: C.bg, overflow: 'hidden' }}>
      ...
      <main style={{
        ...
        {/* Mobile top bar — shown for EVERY mobile view, including full-bleed
            ones (Tools/Clients/Playbook). It carries the only hamburger, so
            without it those views had no way back to the main sidebar. */}
        {isMobile && (
          <div style={{
