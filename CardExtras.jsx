...
// ── Phase 9: Calendar ────────────────────────────────────────────────────────
export const listCalendarEvents     = (params = {}) => req('calendar-list-events',  { method: 'POST', body: JSON.stringify(params) });
export const createCalendarEvent    = (data)        => req('calendar-create-event', { method: 'POST', body: JSON.stringify(data) });
export const listTeamCalendarEvents = (params = {}) => req('team-calendar-events', { method: 'POST', body: JSON.stringify(params) });
...
// ── Phase 11: Playbook (Notion-backed) ──────────────────────────────────────
export const getPlaybookTree          = (rootId)  => req(`playbook-tree${rootId ? `?rootId=${encodeURIComponent(rootId)}` : ''}`);
export const getPlaybookPage          = pageId    => req(`playbook-page?pageId=${encodeURIComponent(pageId)}`);
export const getPlaybookBlockChildren = blockId   => req(`playbook-block-children?blockId=${encodeURIComponent(blockId)}`);
export const savePlaybookBlock        = data      => req('playbook-save-block', { method: 'POST', body: JSON.stringify(data) });

// ── Playbook links — multi-page Notion playbook (§3): iframe + edit-in-Notion ──
export const listPlaybookLinks  = (company = 'global') => req(`playbook-links-list?company=${encodeURIComponent(company)}`);
export const upsertPlaybookLink = (data)               => req('playbook-links-upsert', { method: 'POST', body: JSON.stringify(data) });
export const deletePlaybookLink = (id)                 => req('playbook-links-delete', { method: 'POST', body: JSON.stringify({ id }) });

// ── Phase 16: Notion OAuth ────────────────────────────────────────────────────
export const startNotionOAuth        = ()   => req('notion-oauth-start', { method: 'POST', body: '{}' });
export const getNotionConnectionStatus = () => req('notion-connection-status');
export const disconnectNotion        = ()   => req('notion-disconnect', { method: 'POST', body: '{}' });
