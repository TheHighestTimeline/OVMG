...
function render() {
  ...
  if (s.noHotspot) {
    ...
    hotspot.style.left = (s.clickX != null ? s.clickX : 50) + '%';
    hotspot.style.top = (s.clickY != null ? s.clickY : 50) + '%';
    positionTooltip(s);
  }
  renderAnnotations(s);
...
function hotspotClick() {
  const s = STEPS[idx];
  if (s && s.typedImage && !s._typed) {
    // Simulate a typing/fill-in transition: swap to the "typed" still frame,
    // then continue to the next step after a brief pause.
    s._typed = true;
    img.src = s.typedImage;
    hotspot.style.display = 'none';
    tooltip.style.display = 'none';
    setTimeout(next, 450);
    return;
